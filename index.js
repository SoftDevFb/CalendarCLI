...
_.forEach(Array.from(year.by('months')), month => {
    ...
    let paddedDays = _.map(days, day => { // Transforms items in the array
        ...
        if (day.month() == 8 && day.date() == 10) {//Can use one month behind to get it to print correct month like this or see below
            date = chalk.bgBlue(date)

        }
        // TODO: Highlight YOUR birthday in color!
        if ((day.month() + 1) == 2 && day.date() == 28) {//Or can add one month to selected month like this
            date = chalk.bgBlue(date)
